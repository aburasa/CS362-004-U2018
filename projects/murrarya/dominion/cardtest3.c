...
void setHand(struct gameState *G, int treasure) {
	int seed = 1000;
	int numPlayers = 2;
	int maxBonus = 10;
...
void testMine() {
	...
	int preDeckSize = G.deckCount[G.whoseTurn];
	result = cardEffect(mine, 0, silver, 0, &G, 1, 0);
	...
	assertTrue(G.playedCardCount == prePlayedCount + 1, "Played card count increased by 1");
	assertTrue(G.playedCards[prePlayedCount] == copper, "Played card is discarded");
	assertTrue(G.deckCount[G.whoseTurn] == preDeckSize - 1, "Deck size decreased by one after success");

	// Test that copper cannot be replaced with gold
	setHand(&G, copper);
	prePlayedCount = G.playedCardCount;
	preDeckSize = G.deckCount[G.whoseTurn];
	...
	assertTrue(G.deckCount[G.whoseTurn] == preDeckSize, "Deck count not changed after failure");
